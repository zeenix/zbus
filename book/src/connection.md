...
zbus's integration tests include a reference host: a single-threaded runtime built on the
...
The blocking API needs one thing of such a host: its loop has to run on a thread other than the
one calling into `zbus::blocking`. A blocking call made from the thread the loop itself runs on
deadlocks, because the connection it waits on only makes progress while that loop runs — which it
cannot do until the call it is blocked in returns.
