...
mod tests {
    ...
    fn connect_abstract_unix() {
        ...
        // The listener is bound through the standard library, so what the test proves is that
        // an `abstract=` address reaches the socket that name really belongs to, rather than
