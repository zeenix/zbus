...
impl<'a> Builder<'a> {
    ...
    ///
    /// The connection watches its socket, takes its timers and runs its internal tasks on
    /// `runtime`. zbus never drives a runtime of its own, so the tasks the connection
