...
use crate::{ByteOrder, Error, Message, Result};

/// The body of a message.
///
/// This contains the bytes and the signature of the body.
#[derive(Clone, Debug)]
pub struct Body<O: ByteOrder> {
    data: Data<'static, 'static, O>,
    msg: Message<O>,
}

impl<O: ByteOrder> Body<O> {
    pub(super) fn new(data: Data<'static, 'static, O>, msg: Message<O>) -> Self {
        Self { data, msg }
    }
    ...
    pub fn data(&self) -> &serialized::Data<'static, 'static, O> {
        &self.data
    }

    /// Reference to the message this body belongs to.
    pub fn message(&self) -> &Message<O> {
        &self.msg
    }
}
