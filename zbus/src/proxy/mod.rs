...
where
    T: crate::wire::Type,
{
    /// Get the value of the property that changed, in serialized form.
    ///
    /// Unlike [`PropertyChanged::get`], the returned [`PropertyData`] allows deserializing into
    /// a type that borrows from it.
    pub async fn get_data(&self) -> Result<PropertyData> {
        self.get_raw()
            .await
            .and_then(|value| PropertyData::new(&value, T::SIGNATURE))
    }
}

/// A property value, serialized but not yet deserialized.
///
/// Returned by [`Proxy::get_property_data`], [`PropertyChanged::get_data`] and the getters
/// generated by [`macro@crate::proxy`] for property types that borrow from the deserializer.
/// It owns the serialized property value, so [`PropertyData::deserialize`] can produce a type
/// borrowing from `self` — the D-Bus reply the value originates from does not outlive the
/// getter call, but `PropertyData` does.
#[derive(Debug)]
pub struct PropertyData {
    data: crate::wire::serialized::Data<'static, 'static>,
}

impl PropertyData {
    fn new(value: &Value<'_>, expected_signature: &crate::wire::Signature) -> Result<Self> {
        as_value::serialized_for_property(value, expected_signature).map(|data| Self { data })
    }

    /// Deserialize the property value, borrowing from `self` where possible.
    pub fn deserialize<'d, T>(&'d self) -> Result<T>
    where
        T: serde::Deserialize<'d> + crate::wire::Type + 'd,
    {
        let value: as_value::Deserialize<'_, T> = self.data.deserialize()?.0;
...
impl<'a> Proxy<'a> {
    ...
    /// be deserialized into a type that borrows from the returned [`PropertyData`].
    pub fn cached_property_data(
        ...
        self.cached_property_raw(property_name)
            .as_deref()
            .map(|value| PropertyData::new(value, expected_signature))
            .transpose()
    }
    ...
    ///
    /// Get the property value from the cache (if caching is enabled) or call the
    /// `Get` method of the `org.freedesktop.DBus.Properties` interface, like
    /// [`Proxy::get_property`], but return it in serialized form. Unlike `get_property`, this
    /// allows deserializing into a type that borrows from the returned [`PropertyData`].
    ///
    /// `expected_signature` is the D-Bus signature of the target type (usually
    /// [`Type::SIGNATURE`]); the serialized value is validated (and where necessary, converted)
    /// against it.
    ///
    /// [`Type::SIGNATURE`]: crate::wire::Type::SIGNATURE
    pub async fn get_property_data(
        &self,
        property_name: &str,
        expected_signature: &crate::wire::Signature,
    ) -> Result<PropertyData> {
        ...
        if let Some(value) = self.cached_property_raw(property_name) {
            return PropertyData::new(&value, expected_signature);
        }

        let value = self.get_proxy_property(property_name).await?;
        PropertyData::new(&value, expected_signature)
    }
