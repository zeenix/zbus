...
// The refused port the connection tests aim at is reserved here, so this module is not private
// to the socket's own tests.
...
pub(crate) struct RegisteredIo<O> {
    ...
    source: IoSource,
    // Kept so that the ops can hand the blocking part of an operation, such as the peer's
