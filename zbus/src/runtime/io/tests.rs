...
/// A connection still being made reads as a would-block, on every platform.
///
/// The socket here has had no connection asked for at all, which is what one the kernel is still
/// working on looks like to the predicate: it has no peer to name. Reporting that as a would-block
/// is what lets the wait for a connection be a readiness wait like any other, so the two answers
/// have to agree even though the error underneath is `ENOTCONN` on unix and `WSAENOTCONN` on
/// Winsock.
#[test]
...
/// leaves a connection pending there, while Winsock refuses it outright instead. The predicate's
/// reading of a socket with no peer, which is what a connection under way looks like, is covered
/// on every platform by [`a_socket_with_no_peer_reads_as_would_block`].
#[cfg(unix)]
#[test]
#[timeout(15000)]
...
/// A peer's supplementary groups are looked up through the runtime, and only where there is a
/// lookup to make.
///
...
fn a_group_lookup_reaches_the_runtime_only_where_there_is_one() {
    ...
    // Only Linux and Android look supplementary groups up, and one call is what that takes.
