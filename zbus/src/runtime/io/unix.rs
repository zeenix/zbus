...
impl GroupLookup {
    ...
    /// them.
    ///
    /// The backend-typed socket impls ask from a thread of a blocking pool they went to
    /// themselves, so there is no runtime of the connection's to hand the lookup to.
