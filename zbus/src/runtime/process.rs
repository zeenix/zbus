...
//! way its sockets are, and waiting for it to exit is blocking work that runtime is handed. So a
//! helper process needs neither a reactor nor a thread of zbus's own, and it belongs to the same
//! runtime as the rest of the connection.
//!
...
mod split_tests {
    ...
    /// read, which is the point at which nothing is waiting for it yet.
    #[test]
    #[timeout(15000)]
