...
//! feature, on by default): one worker thread and one wake pipe per connection, and no runtime
//! dependency of its own. A connection built inside a Tokio runtime runs on it instead (the
//! `tokio` feature), sparing itself that extra thread, and any other runtime reaches a
